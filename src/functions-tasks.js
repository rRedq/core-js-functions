...
function getFunctionBody(func) {
  if (!func) return '';
  return func.prototype.constructor;
}
...
function getPolynom(...rest) {
  ...
  if (arguments.length === 1) {
    ...
    result = (x) => rest[0] * x + rest[1];
  } else if (arguments.length === 3) {
    result = (x) => rest[0] * x ** 2 + rest[1] * x + rest[2];
  } else {
    result = null;
  }
  return result;
...
function logger(func, logFunc) {
  return (...rest) => {
    const args = rest.map((item) => JSON.stringify(item));
...
function getIdGeneratorFunction(startFrom) {
  let count;
  return () => {
    if (!count && count !== startFrom) count = startFrom;
    else count += 1;
    return count;
  };
}
